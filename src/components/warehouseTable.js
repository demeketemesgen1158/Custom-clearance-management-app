...
export default function WarehouseTable({tWidth, intialDate, EndDate, awb}) {
  ...
  // );
  const filteredData = rows.filter(fRow=>((new Date(iDate)>new Date('2000-01-01'))?(iDate<=new Date(fRow.issueDate) && eDate>=new Date(fRow.issueDate)):true && ((awb>0)?fRow.awb===awb:true) && (fRow.status==='Delivered' || fRow.status==='Pending') && Number(fRow.no)<(rows.length)));
  
  let totalFee = filteredData.reduce((sum, p)=>(Number(sum) + Number(p.warehouseFee)), 0);
              ...
              {filteredData.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left" sx={{textWrap:'noWrap'}}>{row.warehouseFee}</TableCell>
                   </TableRow>
                );
              })}
              {/* {emptyRows > 0 && (
